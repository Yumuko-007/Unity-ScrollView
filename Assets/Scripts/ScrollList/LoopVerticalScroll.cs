...
namespace Game
{
    public class LoopVerticalScroll : ScrollRect, IPointerDownHandler
    {
        ...
        public void ShowAtBottom()
        {
            if (_items.Count > 0)
            {
                ...
                else
                {
                    ...
                    float lastPadding = _items[_items.Count - 1].GetPaddingToBottomViewSize(_objectCount);
        ...
        private LoopItem Spwan(int idx)
        {
            ...
            item.Init(content, _viewportItem, idx, _padding, _spacingNum, LoopObjectType.Vertical);
        ...
        public void OnPointerDown(PointerEventData eventData)
        {
            ...
            verticalNormalizedPosition = 0.5f;
            float delta = content.anchoredPosition.y - y;
            foreach (var item in _items)
            {
        ...
        private bool MovementElasticHandle(PointerEventData eventData)
        {
            ...
            if (first.Index == 0)
            {
                float delta = first.GetPaddingToTopViewSize();
                if (delta > 0)
                {
                    if (movementType == MovementType.Elastic)
                    {
                        RectTransformUtility.ScreenPointToLocalPointInRectangle(content, eventData.position, eventData.pressEventCamera, out Vector2 p1);
                        RectTransformUtility.ScreenPointToLocalPointInRectangle(content, eventData.position - eventData.delta, eventData.pressEventCamera, out Vector2 p2);
            ...
            {
                float delta = last.GetPaddingToBottomViewSize(_objectCount);
                if (delta > 0)
                {
                    if (movementType == MovementType.Elastic)
                    {
        ...
        private void MovementRecovery()
        {
            ...
            if (_isClamping || velocity != Vector2.zero)
            {
                if (_items[0].Index == 0)
                {
                    float delta = _items[0].GetPaddingToTopViewSize();
                    if (delta > 0.001f)
                    {
                        StopMovement();
                        _isClamping = true;     // 大力拖拽后，靠速度触发，此时暂停移动需要设置标志
                        Vector3 localPos = content.localPosition;
                        Vector3 conenteEndPos = localPos + new Vector3(0, delta, 0);
                ...
                {
                    float delta = _items[_items.Count - 1].GetPaddingToBottomViewSize(_objectCount);
                    if (delta > 0.001f)
                    {
                        StopMovement();
                        _isClamping = true;     // 大力拖拽后，靠速度触发，此时暂停移动需要设置标志
