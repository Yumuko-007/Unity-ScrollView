...
namespace Game
{
    public class LoopVerticalScroll : LoopScroll, IPointerDownHandler, ILoopScroll
    {
        ...
        private List<LoopVerticalItem> _items = new(); // 显示列表
        private Queue<LoopVerticalItem> _pool = new(); // 对象池
        ...
        public void InitLoop(int count, Func<int, ILoopObject> showAction, int startIndex = 0, bool atTop = true)
        {
            if (_padding == null)
            {
                ...
                if (verticalLayoutGroup != null && verticalLayoutGroup.enabled)
                {
                    ...
                    _padding = verticalLayoutGroup.padding;
                    _spacingNum = verticalLayoutGroup.spacing;
                    _childAligin = verticalLayoutGroup.childAlignment;
                }
                else
                {
            ...
            content.sizeDelta = new Vector2(content.sizeDelta.x, contentSize * 2);
        ...
        /// 获取制定位置的循环对象
        /// </summary>
        private LoopVerticalItem GetLoopItem(int index)
        {
        ...
        public void ShowAtBottom()
        {
            if (_items.Count > 0)
            {
                ...
                else
                {
                    ...
                    var lastPadding = _items[_items.Count - 1].GetPaddingToBottomViewSize(_objectCount);
        ...
        private void LoopHandleCreate(bool isInit)
        {
            ...
            {
                var first = _items[0];
                var topPadding = first.GetPaddingToTopViewSize();
                if (topPadding > 0)
                {
            ...
            for (var i = 0; i < 10; i++)
            {
                ...
                var bottomPadding = last.GetPaddingToBottomViewSize(_objectCount);
                if (bottomPadding > 0)
                {
        ...
        private void LoopHandleRecycle(bool isInit)
        {
            ...
            for (var i = 0; i < 100 && _items.Count >= 2; i++)
            {
                ...
                if (status == LoopObjectStatus.UpOut || status == LoopObjectStatus.DownOut)
                {
                    Despwan(_items[0]);
                    continue;
                }
        ...
        public int GetFirstIndexInView(LoopObjectStatus type = LoopObjectStatus.Full)
        {
            ...
            {
                var curType = item.GetObjectViewStatus();
                if (curType == type) return item.Index;
            }
        ...
        /// 生成对象
        /// </summary>
        private LoopVerticalItem Spwan(int idx)
        {
            if (_items.Count >= 400) Debug.LogError("数据异常！！！！");

            var item = _pool.Count > 0 ? _pool.Dequeue() : new LoopVerticalItem();
        ...
        /// 回收
        /// </summary>
        private void Despwan(LoopVerticalItem item)
        {
            _pool.Enqueue(item);
            _items.Remove(item);
            item.DespawnSelf();
        ...
        public void OnPointerDown(PointerEventData eventData)
        {
            ...
            verticalNormalizedPosition = 0.5f;
            var delta = content.anchoredPosition.y - y;
        ...
        private bool MovementElasticHandle(PointerEventData eventData)
        {
            ...
            if (first.Index == 0)
            {
                var delta = first.GetPaddingToTopViewSize();
                if (delta > 0)
                {
                    if (movementType == MovementType.Elastic)
                    {
                        ...
                        content.anchoredPosition += new Vector2(0, (p1.y - p2.y) * elasticity);
                    }
                    else
                    {
            ...
            {
                var delta = last.GetPaddingToBottomViewSize(_objectCount);
                if (delta > 0)
                {
                    if (movementType == MovementType.Elastic)
                    {
        ...
        private void MovementRecovery()
        {
            ...
            if (_isClamping || velocity != Vector2.zero)
            {
                if (_items[0].Index == 0)
                {
                    var delta = _items[0].GetPaddingToTopViewSize();
                    if (delta > 0.001f)
                    {
                        StopMovement();
                        _isClamping = true; // 大力拖拽后，靠速度触发，此时暂停移动需要设置标志
                        var localPos = content.localPosition;
                        var conenteEndPos = localPos + new Vector3(0, delta, 0);
                ...
                {
                    var delta = _items[_items.Count - 1].GetPaddingToBottomViewSize(_objectCount);
                    if (delta > 0.001f)
                    {
                        StopMovement();
                        _isClamping = true; // 大力拖拽后，靠速度触发，此时暂停移动需要设置标志
